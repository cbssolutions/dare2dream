...
dump_db_without_list_db_check = db.dump_db.__wrapped__
...
class DbBackup(models.Model):
    ...
    _sql_constraints = [
        ...
        (
            ...
            "CHECK(days_to_keep >= 0 and cbs_only_day_divisible_with > 0)",
            "You must have days_to_keep >= 0 and cbs_only_day_divisible_with > 0.",
        ),
    ]
    ...
    cbs_weeks_to_keep = fields.Integer(
        tracking=1,
        required=True,
        default=0,
        help="Backups older than this will be deleted automatically. "
    ...
    cbs_months_to_keep = fields.Integer(
        tracking=1,
        required=True,
        default=0,
        help="Backups older than this will be deleted automatically. "
    ...
    cbs_only_day_divisible_with = fields.Integer(default=1, help="Will make cron backup only when the days are divisible with this number"
                                             "(not today%cbs_only_day_divisible_with; or if this is 12, will make a backup only if day=12 or 24)."
                                             "Usefull to make full backup only once per week.")
    method = fields.Selection(
        [("local", "Local disk"), ("sftp", "Remote SFTP server")],
        default="local", tracking=1,
    ...
    def action_sftp_test_connection(self):
        ...
        try:
            ...
            _logger.info("Connection Test Failed!", ex)
            raise UserError(_(f"Connection Test Failed! {ex}"))
        else:  # no exception
    ...
    def action_backup(self):
        ...
        for rec in self.filtered(lambda r: r.method == "local"):
            filename = self.filename(datetime.now(), ext=rec.backup_format, database=rec.env.cr.dbname)
            with rec.backup_log():
                ...
                with open(os.path.join(rec.folder, filename), "wb") as destiny:
                    ...
                    # Generate new backup
                        ...
                        dump_db_without_list_db_check(self.env.cr.dbname, destiny, backup_format=rec.backup_format)
        ...
        if sftp:
            for rec in sftp:
                ...
                with rec.backup_log():
                    ...
                    dump_db_without_list_db_check(self.env.cr.dbname, None, backup_format=rec.backup_format)
    ...
    def action_backup_all(self):
        ...
        this_day = fields.Date.today().day
        return self.search([]).filtered(lambda r: not this_day % r.only_day_divisible_with).action_backup()
    ...
    def backup_log(self):
        ...
        try:
            ...
            self.message_post(body=_(f"Database {self.env.cr.dbname} backup succeeded on {self.name}."))
    ...
    def cleanup(self):
        ...
        for rec in self.filtered("days_to_keep"):
            with rec.cleanup_log():
                ...
                oldest = self.filename(now - timedelta(days=rec.days_to_keep), bu_format, database="")

                if rec.method == "local":
                    for name in iglob(os.path.join(rec.folder, "*.{file_extension}")):
                    ...
                    with rec.sftp_connection() as remote:
                        for name in remote.listdir(rec.folder):
                            if (name.endswith(f".{file_extension}") and os.path.basename(name) < oldest):
    ...
    @staticmethod
    def filename(when, ext="zip", database=""):
        ...
        """
        return f'{when:%Y%m%d_%H_%M_%S}_{database}.{"dump.zip" if ext == "zip" else ext}'
